...
plots_dir = f'{ATTACK_NAME}/{MODEL_NAME}/plots'
results_dir = f'{ATTACK_NAME}/{MODEL_NAME}/results'
models_dir = f'{ATTACK_NAME}/{MODEL_NAME}/models'
...
data[[f'{i}' for i in range(10)]] = 1*pd.get_dummies(data['fault'])
data.pop('fault')
data

X= data.drop([f'{i}' for i in range(10)], axis=1)
y = data[[f'{i}' for i in range(10)]]
...
y_test = convert_y(y_test.values)
...
train_pred = convert_y(train_pred)
y_train = convert_y(y_train.values)
...
model.save(f'{models_dir}/model.joblib')
